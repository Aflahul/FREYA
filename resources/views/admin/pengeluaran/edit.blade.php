    ...
    <div class="fixed inset-0 flex justify-center bg-gray-900 bg-opacity-50">
        <div class="p-24 sm:ml-72">
            <div class="max-w-screen-sm mx-auto p-4 bg-white rounded-md border-blue-600">
                <h2 class="text-lg font-medium mb-4">Edit Pengeluaran</h2>
                <form action="{{ route('UpdatePengeluaran.update', $pengeluaran->id_pengeluaran) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="kd_pengeluaran" name="kd_pengeluaran"
                            value="{{ old('kd_pengeluaran', $pengeluaran->kd_pengeluaran) }}"
                            class="border rounded-md px-2 py-1">
                    </div>
                    <div class="mb-4">
                        <label for="pengeluaran" class="block">pengeluaran:</label>
                        <input type="text" id="pengeluaran" name="pengeluaran"
                            value="{{ old('pengeluaran', $pengeluaran->pengeluaran) }}" class="border rounded-md px-2 py-1">
                    </div>
                    <div class="mb-4">
                        <label for="desk" class="block">desk:</label>
                        <input type="text" id="desk" name="desk" value="{{ old('desk', $pengeluaran->desk) }}"
                            class="border rounded-md px-2 py-1">
                    </div>
                    <div class="mb-4">
                        <label for="waktu" class="block">waktu:</label>
                        <input type="datetime-local" id="waktu" name="waktu" value="{{ old('waktu', $pengeluaran->waktu) }}"
                            class="border rounded-md px-2 py-1">
                    </div>
                    <div class="mb-4">
                        <label for="jumlah" class="block">jumlah:</label>
                        <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah', $pengeluaran->jumlah) }}"
                            class="border rounded-md px-2 py-1">
                    </div>
            </div>
